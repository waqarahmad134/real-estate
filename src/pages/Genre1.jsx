...
export default function Genre1() {
  ...
  console.log("🚀 ~ Genre1 ~ categories:", categories)
  console.log("🚀 ~ Genre1 ~ Id:", Id)
  ...
  const fetchMoviesByGenre = useCallback(async () => {
    setLoading(true)
    try {
      const response = await axios.get(`${BASE_URL}all-movies`)
      setMovies(response?.data?.data?.movies)
      console.log("🚀 ~ fetchMoviesByGenre ~ response?.data?.data?.movies:", response?.data?.data)
            ...
            <>
              {movies?.length > 0 ? (
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2 sm:gap-4">
                  {movies?.filter((data =>data?.year === Id))?.map((movie, index) => (
                    <Card4
                      key={index}
                      id={movie.id}
                      slug={movie.slug}
